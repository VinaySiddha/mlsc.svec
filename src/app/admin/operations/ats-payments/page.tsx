...
export default async function AtsPaymentsPage() {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 dark:text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-1">
          ATS Credit Payments Verification Console
        </p>
      </div>
      <Suspense fallback={
        <div className="flex flex-col items-center justify-center py-12 text-slate-400 dark:text-zinc-500">
          ...
          <p className="text-xs font-bold uppercase tracking-wider">Loading Payments Ledger...</p>
        </div>
      }>
        <OperationsCenter mode="ats-payments" />
      </Suspense>
    </div>
  );
}
